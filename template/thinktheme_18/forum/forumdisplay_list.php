...
<div id="threadlist" class="tl"{if $_G['uid']} style="position: relative;"{/if}>
	...
	<div class="th tt_juhe">
        <div class="y" style="*width:310px;_width:310px;">
        	<span id="atarget" style="margin-top:14px; margin-left:10px;" {if $_G['cookie']['atarget'] > 0}onclick="setatarget(-1)" class="y atarget_1"{else}onclick="setatarget(1)" class="y"{/if} title="{lang new_window_thread}">{lang new_window}</span>
            <!--{if !empty($_G['forum']['picstyle'])}-->
        	<a{if empty($_G['cookie']['forumdefstyle'])} href="forum.php?mod=forumdisplay&fid=$_G[fid]&forumdefstyle=yes" class="y chked"{else} href="forum.php?mod=forumdisplay&fid=$_G[fid]&forumdefstyle=no" class="y unchk"{/if}  style="margin-top:14px; margin-left:20px;" title="{lang view_thread_imagemode}{lang view_thread}">{lang view_thread_imagemode}</a>
            <!--{/if}-->
			...
			<table summary="forum_$_G[fid]" cellspacing="0" cellpadding="0" id="threadlisttableid">
				<!--{if (!$simplestyle || !$_G['forum']['allowside'] && $page == 1) && !empty($announcement)}-->
					<tbody>
						<tr>
							<td class="icn">
                                <!--{if 0}--><img src="{IMGDIR}/ann_icon.gif" alt="{lang announcement}" /><!--{/if}-->
                            </td>
							<!--{if $_G['forum']['ismoderator'] && !$_GET['archiveid']}--><td class="o">&nbsp;</td><!--{/if}-->
							<th class="common"><strong class="xst">{lang announcement}: <!--{if empty($announcement['type'])}--><a href="forum.php?mod=announcement&id=$announcement[id]#$announcement[id]" target="_blank">$announcement[subject]</a><!--{else}--><a href="$announcement[message]" target="_blank">$announcement[subject]</a><!--{/if}--></strong></th>
							<td class="by">
							</td>
							<td class="num">&nbsp;</td>
							<td class="by">&nbsp;</td>
				...
				<!--{if 0}-->
					<tbody id="separatorline" class="emptb"><tr>
                        <td class="icn"></td>
                        <!--{if !$_GET['archiveid'] && $_G['forum']['ismoderator']}--><td class="o"></td><!--{/if}--><th></th><!--{if CURMODULE == 'guide'}--><td class="by"></td><!--{/if}--><td class="by"></td><td class="num"></td><td class="by"></td></tr>
                    </tbody>
				<!--{/if}-->
								...
								<tr>
									...
									<th class="$thread[folder]">
                                        <div class="avatarbox">
                                            ...
                                            <!--{if 0 && $thread['authorid'] && $thread['author']}-->
                                                <!--{if !empty($verify[$thread['authorid']])}-->
                                                <div class="veri_icon">$verify[$thread[authorid]]</div>
                                                <!--{/if}-->
                                            <!--{/if}-->
                                        </div>
                                        ...
                                        <div class="tl_ct">
                                            ...
                                            <!--{/if}-->
                                            <a href="forum.php?mod=viewthread&tid=$thread[tid]&extra=$extra" class="y views">
                                                <!--{if $thread['isgroup'] != 1}-->
                                                $thread[views]
                                                <!--{else}-->
                                            ...
                                            <!--{if $thread['displayorder'] == 0}-->
                                                ...
                                                <!--{/if}-->
                                                <!--{if $thread[heatlevel]}-->
                                                    <img src="$_G['style']['styleimgdir']/hot_$thread[heatlevel].gif" align="absmiddle" alt="heatlevel" title="{lang heats}: {$thread[heats]}" />
                                                <!--{/if}-->
                                                <!--{if $thread['rate'] > 0}-->
                                            ...
                                            <!--{/if}-->
                                            <div class="info">
                                                <em class="y replies">$thread[allreplies]</em>
                                                <!--{hook/forumdisplay_author $key}-->
                                                <em>
                                                ...
                                                <em>于 <span{if $thread['istoday']} class="xi1"{/if}>$thread[dateline]</span> 发表<!--{if $thread[typename]}-->在 <!--{$thread[typehtml]}--><!--{/if}--><!--{if $groupnames[$thread[tid]]}-->
                                                    <span class="xg1">[<a href="forum.php?mod=forumdisplay&fid={$groupnames[$thread[tid]][fid]}" target="_blank" class="xg1">{$groupnames[$thread[tid]][name]}</a>]</span>
                                                <!--{/if}--></em>
                                                <em>最后回复
                                                ...
                                                <a href="forum.php?mod=viewthread&tid=$thread[icontid]&{if $_GET['archiveid']}archiveid={$_GET['archiveid']}&{/if}extra=$extra" target="_blank">
                                                    {if $thread['displayorder'] == 1}置顶{/if}
                                                    {if $thread['displayorder'] == 2}置顶{/if}
                                                    {if $thread['displayorder'] == 3}置顶{/if}
                                                    {if $thread['displayorder'] == 4}置顶{/if}
                                                    {if $thread['closed'] == 1}关闭{/if}
                                                <!--{if 0}-->
                                                    {if $thread['special'] == 1}{lang thread_poll}{/if}
                                                    {if $thread['special'] == 2}{lang thread_trade}{/if}
                                                    {if $thread['special'] == 3}{lang thread_reward}{/if}
...
<!--{if $multipage && $filter != 'hot'}-->
	<!--{if !($_G[forum][picstyle] && !$_G[cookie][forumdefstyle])}-->
        <div id="pgbtn" class="pgbtn">
		    <a class="bm_h" href="javascript:;" rel="$multipage_more" curpage="$page" id="autopbn" totalpage="$maxpage" picstyle="$_G[forum][picstyle]" forumdefstyle="$_G[cookie][forumdefstyle]">{if 0}{lang next_page_extra}{/if}<i></i>加载更多</a>
		    <script type="text/javascript" src="{$_G[setting][jspath]}autoloadpage.js?{VERHASH}"></script>
        </div>
	<!--{else}-->
		<div id="pgbtn" class="pgbtn">
            <a href="forum.php?mod=forumdisplay&fid={$_G[fid]}&filter={$filter}&orderby={$_GET[orderby]}{$forumdisplayadd[page]}&{$multipage_archive}&page=$nextpage" hidefocus="true">{lang next_page_extra}</a>
        </div>
	<!--{/if}-->
<!--{/if}-->
