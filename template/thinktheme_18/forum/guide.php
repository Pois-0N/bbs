...
<div class="boardnav">
	<div id="ct" class="wp cl{if $_G['forum']['allowside']} ct2{/if}"{if $leftside} style="margin-left:{$_G['leftsidewidth_mwidth']}px"{/if}>
		<div class="mn">
			...
			<!--{else}-->
				<!--{loop $data $key $list}-->
				<div id="threadlist" class="tl pdd10 bm bmw"{if $_G['uid']} style="position: relative;"{/if}>
                <!--{if $view != 'index'}-->
			<div id="pgt" class="bm bw0 pgs cl">
				$multipage
				<a class="pn pnc" style="padding:2px 10px" onclick="showWindow('nav', this.href, 'get', 0)" href="forum.php?mod=misc&amp;action=nav">{lang send_posts}</a>
			</div>
			<!--{/if}-->
					<div class="th br4" style="padding:10px;">
