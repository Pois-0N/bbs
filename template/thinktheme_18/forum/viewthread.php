...
<div id="pt" class="bm cl">
	<div class="z">
        ...
        $navigation
        <!--<em>&rsaquo;</em>-->
        <!--<a href="forum.php?mod=viewthread&tid=$_G[tid]">$_G[forum_thread][short_subject]</a>-->
	</div>
</div>
        ...
        <div class="tt_p_tp">
            <div class="y">
                <div class="tt_tp_action">
                    <!--{if !$close_leftinfo}-->
                        <span class="y replies">$_G[forum_thread][replies]</span>
                        <span class="y views">$_G[forum_thread][views]</span>
                    <!--{/if}-->
                </div>
            </div>
                ...
                <!--{if !$close_leftinfo}-->
                    ...
                    <a href="home.php?mod=space&uid=$_G[forum_thread][authorid]" title="$_G[forum_thread][author]" class="xm_avt" {if !($_G['setting']['threadguestlite'] && !$_G['uid'])} onmouseover="showauthor(this, 'userinfo$_G[forum_thread][authorid]')"{/if}>
                        <!--{avatar($_G[forum_thread][authorid],small)}-->
                    </a>

                    <h1>
                        <a href="forum.php?mod=viewthread&tid=$_G[tid]$fromuid" id="thread_subject">$_G[forum_thread][subject]</a>
                        ...
                        &nbsp;<img class="vm" src="{IMGDIR}/recommend_$_G['forum_thread']['recommendlevel'].gif" alt="" title="{lang thread_recommend} $_G['forum_thread'][recommends]" />
                        <!--{/if}-->

                        <!--{if $_G['forum_thread'][heatlevel]}-->
                        &nbsp;<img class="vm" src="{IMGDIR}/hot_$_G['forum_thread'][heatlevel].gif" alt="" title="{lang heats}: $_G['forum_thread']['heats']" />
                        <!--{/if}-->

                        <!--{if $_G['forum_thread']['closed'] == 1}-->
                        &nbsp;<img class="vm" src="{IMGDIR}/locked.gif" alt="{lang close}" title="{lang close}" />
                        <!--{/if}-->
                        <!--{/if}-->
                    </h1>
                    ...
                    <div class="tt_tp_action">
                        <span>
                            ...
                            <!--{if $member_verify[$_G[forum_thread][authorid]]['verify1'] == 1}-->
                            <a href="http://www.adminbuy.cn" target="_blank">
                                <img src="$_G['setting']['verify']['1'][icon]" class="vm" alt="$_G['setting']['verify']['1']['title']" title="$_G['setting']['verify']['1']['title']" />&nbsp;
                            </a>
                            <!--{/if}-->
                        </span>

                        <span>于 <!--{eval echo dgmdate($_G['thread']['dateline'], 'u');}--></span>

                        <span>发表<!--{if $_G['forum_thread']['typeid'] && $_G['forum']['threadtypes']['types'][$_G['forum_thread']['typeid']]}-->在
                        ...
                        <!--{if $threadsorts && $_G['forum_thread']['sortid']}-->
                            <span><a href="forum.php?mod=forumdisplay&fid=$_G[fid]&filter=sortid&sortid=$_G[forum_thread][sortid]">[{$_G['forum']['threadsorts']['types'][$_G['forum_thread']['sortid']]}]</a></span>
                        <!--{/if}-->

                        <span>
                            {if $thread['displayorder'] == 1}<em style="color: #ed5669;">置顶</em>{/if}
                            {if $thread['displayorder'] == 2}<em style="color: #ed5669;">置顶</em>{/if}
                            {if $thread['displayorder'] == 3}<em style="color: #ed5669;">置顶</em>{/if}
                            {if $thread['displayorder'] == 4}<em style="color: #ed5669;">置顶</em>{/if}

                            {if $thread['closed'] == 1}<em style="color: #5e9dec;">关闭</em>{/if}
                        </span>
                    ...
                    <div class="tt_tp_action">
                        ...
                        {lang anonymous}
                        <!--{/if}-->
                        {lang poston} <!--{eval echo dgmdate($_G['thread']['dateline'], 'u');}-->
                    <!--{/if}-->
                        <span class="xg1">
                            <a href="forum.php?mod=viewthread&tid=$_G[tid]$fromuid" onclick="return copyThreadUrl(this, '$_G[setting][bbname]')" title="拷贝{$_G['forum_thread']['subject']}链接">[{lang share_url_copy}]</a>
						
                            <!--{if !IS_ROBOT}-->
                                <!--{if $post['invisible'] == 0}-->

                                    <!--{if !$_GET['authorid'] && !$_G['forum_thread']['archiveid']}-->
                                        <a href="forum.php?mod=viewthread&tid=$post[tid]&page=$page&authorid=$_G[forum_thread][authorid]" rel="nofollow">[<!-- {lang thread_show_author} -->只看楼主]</a>
                                    <!--{elseif !$_G['forum_thread']['archiveid']}-->
                                        <a href="forum.php?mod=viewthread&tid=$post[tid]&page=$page" rel="nofollow">[{lang thread_show_all}]</a>
                                    <!--{/if}-->

                                <a href="forum.php?mod=viewthread&action=printable&tid=$_G[tid]" title="{lang thread_printable}" target="_blank">
                                    [打印]
                                </a>
                                <!--{/if}-->

                                <a href="forum.php?mod=redirect&goto=nextoldset&tid=$_G[tid]" title="{lang last_thread}">
                                    [{lang last_thread}]
                                </a>
                                <a href="forum.php?mod=redirect&goto=nextnewset&tid=$_G[tid]" title="{lang next_thread}">
                                    [{lang next_thread}]
                                </a>
                            <!--{/if}-->
        ...
        <div id="postlist" class="pl">
            ...
            <!--{loop $postlist $post}-->
                ...
                <div id="post_$post[pid]" {if $_G['blockedpids'] && $post['inblacklist']}style="display:none;"{/if}>
                    <!--{if $post[first]}-->
					<!--{eval $firstpost = $post;}-->
                        <!--{subtemplate forum/viewthread_lz_node}-->
                    <!--{else}-->
...
<!-- 发表主题-->
			...
			<div class="bm" style="margin-bottom:10px;">
					<a class="post_btn br4" style="width:230px;" href="javascript:;" id="newspecial" {if !$_G['forum']['allowspecialonly'] && empty($_G['forum']['picstyle']) && !$_G['forum']['threadsorts']['required']} onclick="showWindow('newthread', 'forum.php?mod=post&action=newthread&fid=$_G[fid]')"{else} onclick="location.href='forum.php?mod=post&action=newthread&fid=$_G[fid]';return false;"{/if}>
					</a>
			</div>
			<!--{/if}-->
