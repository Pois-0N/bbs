...
<!--{if empty($topic) || ($topic[usefooter])}-->
	...
	<div id="ft" class="wp cl">
		...
		<!--{/if}-->
        <p class="tt_footlogo"><img src="$_G['style']['styleimgdir']/footerbg.png" /></p>
	</div>
<!--{/if}-->
...
<div id="scrolltop" class="br4" style="visibility:hidden;">
	...
	<span><a href="forum.php?mod=post&action=reply&fid=$_G[fid]&tid=$_G[tid]&extra=$_GET[extra]&page=$page{if $_GET[from]}&from=$_GET[from]{/if}" onclick="showWindow('reply', this.href)" class="replyfast" id="uc_replyfast" title="{lang fastreply}">{lang fastreply}</a></span>
    <span>
             <a href="javascript:;" id="newspecial" {if !$_G['forum']['allowspecialonly'] && empty($_G['forum']['picstyle']) && !$_G['forum']['threadsorts']['required']} onclick="showWindow('newthread', 'forum.php?mod=post&action=newthread&fid=$_G[fid]')"{else} onclick="location.href='forum.php?mod=post&action=newthread&fid=$_G[fid]';return false;"{/if} title="快速发帖" class="postfast">快速发帖</a> 
    </span>
    <span hidefocus="true"><a title="{lang scrolltop}" onclick="window.scrollTo('0','0')" class="scrolltopa" >{lang scrolltop}</a></span>
    <!--{/if}-->
	<!--{if $_G[fid]}-->
	<span>
		<!--{if $_G['mod'] == 'viewthread'}-->
		<a href="forum.php?mod=forumdisplay&fid=$_G[fid]" hidefocus="true" class="returnlist" title="{lang return_list}">{lang return_list}</a>
		<!--{elseif $_G[fid] == 999}-->
        <span><a id="newspecial" onclick="showWindow('nav', this.href);return false;" href="forum.php?mod=misc&amp;action=nav&amp;special=0&amp;" title="快速发帖" class="postfast">快速发帖</a></span>
        <a href="forum.php" hidefocus="true" class="returnlist" title="{lang return_forum}">{lang return_forum}</a>
        <span hidefocus="true"><a title="{lang scrolltop}" onclick="window.scrollTo('0','0')" class="scrolltopa" >{lang scrolltop}</a></span>
        <!--{else}-->
        <span><a href="javascript:;" id="newspecial" {if !$_G['forum']['allowspecialonly'] && empty($_G['forum']['picstyle']) && !$_G['forum']['threadsorts']['required']} onclick="showWindow('newthread', 'forum.php?mod=post&action=newthread&fid=$_G[fid]')"{else} onclick="location.href='forum.php?mod=post&action=newthread&fid=$_G[fid]';return false;"{/if} title="快速发帖" class="postfast">快速发帖</a></span>
		<a href="forum.php" hidefocus="true" class="returnlist" title="{lang return_forum}">{lang return_forum}</a>
        <span hidefocus="true"><a title="{lang scrolltop}" onclick="window.scrollTo('0','0')" class="scrolltopa" >{lang scrolltop}</a></span>
		<!--{/if}-->
	</span>
