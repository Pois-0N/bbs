...
<div id="ct" class="wp cl">
	<div id="pgt" class="pgs mbm cl {if $modmenu['thread']}pbm bbs{/if}">
		...
		<!--{if $allowpostreply && !$_G['forum_thread']['archiveid']}-->
			<a id="post_reply" onclick="showWindow('reply', 'forum.php?mod=post&action=reply&fid=$_G[fid]&tid=$_G[tid]')" href="javascript:;" title="{lang reply}"><img src="$_G['style'][styleimgdir]/pn_reply.png" alt="{lang reply}" /></a>
		<!--{/if}-->
...
<div id="postlist" class="pl bm">
				...
				<h1 style="color: #555555; font-size: 18px; font-weight: 600;">
					<!--{if $_G['forum_thread']['typeid'] && $_G['forum']['threadtypes']['types'][$_G['forum_thread']['typeid']]}-->
						<!--{if !IS_ROBOT && ($_G['forum']['threadtypes']['listable'] || $_G['forum']['status'] == 3)}-->
							<a href="forum.php?mod=forumdisplay&fid=$_G[fid]&filter=typeid&typeid=$_G[forum_thread][typeid]" style="color: #555555; font-size: 18px; font-weight: 600;">[{$_G['forum']['threadtypes']['types'][$_G['forum_thread']['typeid']]}]</a>
						<!--{else}-->
							[{$_G['forum']['threadtypes']['types'][$_G['forum_thread']['typeid']]}]
						<!--{/if}-->
					<!--{/if}-->
					<!--{if $threadsorts && $_G['forum_thread']['sortid']}-->
						<a href="forum.php?mod=forumdisplay&fid=$_G[fid]&filter=sortid&sortid=$_G[forum_thread][sortid]" style="color: #555555; font-size: 18px; font-weight: 600;">[{$_G['forum']['threadsorts']['types'][$_G['forum_thread']['sortid']]}]</a>
					<!--{/if}-->
					<span id="thread_subject" style="color: #555555; font-size: 18px; font-weight: 600;">$_G[forum_thread][subject]</span>
				</h1>
	...
	<!--{loop $postlist $post}-->
		<!--{if $rushreply && $_GET['checkrush'] && $post['rewardfloor'] != 1}-->
			<!--{eval continue;}-->
		<!--{/if}-->
		<div id="post_$post[pid]" {if $_G['blockedpids'] && $post['inblacklist']}style="display:none; border-bottom: 1px solid #EEEEEE;"{/if}>
	...
	<!--{/if}-->
	<!--{if $allowpostreply && !$_G['forum_thread']['archiveid']}-->
		<a id="post_replytmp" onclick="showWindow('reply', 'forum.php?mod=post&action=reply&fid=$_G[fid]&tid=$_G[tid]')" href="javascript:;" title="{lang reply}"><img src="$_G['style'][styleimgdir]/pn_reply.png" alt="{lang reply}" /></a>
	<!--{/if}-->
</div>
