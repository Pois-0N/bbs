    ...
    <i class="arr"></i>
		<div class="pi"<!--{if $close_leftinfo && !$post['anonymous'] && $postshowavatars && $showavatars}--> style="height:48px"<!--{/if}-->>
			...
			<div class="pti">
				...
				<!--{if $_G['setting']['authoronleft']}-->
                   <a href="home.php?mod=space&uid=$post['authorid']" target="_blank" {if $post[groupcolor]} style="color: $post[groupcolor];"{/if}>$post[author]</a>
				<!--{/if}-->
				...
				<!--{if $post['authorid'] && !$post['anonymous']}-->
					...
					<em id="authorposton$post[pid]" style="padding: 0 0 0 10px;">{lang poston} $post[dateline]</em>
					<!--{if $post['status'] & 8}-->
