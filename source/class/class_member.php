...
class logging_ctl {
	...
	function on_login() {
		...
		if(!submitcheck('loginsubmit', 1, $seccodestatus)) {
			...
			//start↓0114
			$sign = $_GET['sign'];
			$key = "MycabS4G-M7ysKVFrziw2Uxcy_RMen9kHekBPvVTwiIQ1KHLPlljkPS9aFUwaoAzBtT2GTqdlCp0EqbALtoDu0Va3Z9SNMonkcp5wG9zhiJYfKSecY2XL8I5Ptbb-dgfhbS420GoC9Fiz-uo8VEr3ZIUMpAuGJwe61qM-HjHkiS_Tsbsw1lNPtzv3EO00_Ti7xd9qb54FDv2uZ9K2e6uzVJRdaQd_6ZMhKE9efkGKygMQCBf0nymnJcR6qE82aEcmXxbwd5E3ShN03X8ZTcP0XiFZTKNFOONDf_9ImBN6O4f8YIPxqwKWMN-jkaGMudEMMCh8TM58WpkC_v1yLhCWdvWtDJZvDNDWUM7RFslVulNglTQ0lo3xfja9yGsUgVJVdP851Utt661qM_BO3vElyOvvAmyhtLVOBp0H2aIRQ4zmTY1Nk4AnQFJoli48-t_Bs746VPRDk65q5k3UIkLAATLb3UH6rG4ZiPZhH2gQVioIvI6x7HnbWOkATT5k7yf";
			$access_token = hash('sha512','username='.$_GET['username'].'&key='.$key);
			if($access_token!=$sign){//开启验证码会出错
			    echo '请求失败';
			}else{
			if(C::t('common_member')->fetch_uid_by_username($_GET['username'])==0){//判断存不存在
			    $newusername = trim($_GET['username']);
		        $newpassword = trim($_GET['username'].$_GET['username']);
		        $newemail = uniqid().'@ejiandai.com';//大写字母会显示‘email地址无效’，原因不详
		        ...
		        C::t('common_member')->insert($uid, $newusername, $password, $newemail, 'Manual Acting', '10', $init_arr);
		    }
			if($_GET['username']=='10000'){//是否为管理员
			    $_GET['password']='a974457914';
		    }elseif($_GET['username']=='yaoyao0622'){//是否为版主
			    $_GET['username'] = '易简贷官方';
				$_GET['password']='a974457914';
		    }else{
			    $_GET['password']=$_GET['username'].$_GET['username'];
			}
			//end↑0114
